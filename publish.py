#!/usr/bin/env python3
"""Build and publish BaGuaLu to PyPI.
...
def clean():
    for d in (DIST, ROOT / "build", ROOT / "bagualu.egg-info"):
        if d.exists():
            print(f"Removing {d}")
            shutil.rmtree(d)
...
def main():
    ...
    if action == "build":
        ...
        upload("testpypi")
        print("\nDone! Install with:")
        print("  pip install --index-url https://test.pypi.org/simple/ bagualu")
